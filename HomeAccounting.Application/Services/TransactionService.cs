...
namespace HomeAccounting.Application.Services
{
	public class TransactionService : ITransactionService
	{
		...
		public async Task CreateTransaction(string token, DateTime createDate, TransactionType type, Guid categoryId, decimal amount, string? title)
		{
			...
			var transaction = Transaction.Create(_guidGenerator.Guid, userId, categoryId, createDate, type, amount, title);
		...
		public async Task<IList<Transaction>> GetTransactionByFiltres(string token, DateTime? startDate, DateTime? endDate, Guid categoryId, TransactionType? transactionType)
		{
			var userId = _jwtProvider.GetUserIdFromToken(token);
			return await _transactionRepository.GetTransactionByFilter(userId,startDate,endDate, categoryId, transactionType);
		}
